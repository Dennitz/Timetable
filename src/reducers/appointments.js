...
export default function reducer(state: AppointmentsState = INITITAL_STATE, action: Action) {
  switch (action.type) {
...
function isAppointmentOfWeek(appointment: Appointment, weekStart: moment) {
  ...
  const appointmentRecursThisWeek = (recurrence === WEEKLY) ||
    ((recurrence === BIWEEKLY) && (
      weekStart.clone()
        .weekday(dayOfAppoinment)
        .diff(startdate, 'weeks') % 2 === 0)
    );

  const currentWeekRange = weekStart.clone().range('week');
  const appointmentRange = moment.range(startdate, enddate);

  return currentWeekRange.overlaps(appointmentRange) && appointmentRecursThisWeek;
}
...
function getTimeOnFixedDate(time: moment$Moment) {
  return time.clone().year(2000).month(1).day(1);
}
...
function insertSortedByTime(
  ...
  const insertIndex = findFirstIndexLaterStart(appointmentArray, appointmentToInsert);
  if (insertIndex === -1) {
 ...
 */
function mapAppointmentsToIndex(appointments: AppointmentsState, weekStart: moment$Moment) {
  const days = Array(7).fill(null).map(() => []);
  R.values(appointments)
    .forEach((appointment) => {
